...
__global__ void farthestpointsamplingKernel(int b, int n, int m, const float * __restrict__ dataset,float * __restrict__ temp, long * __restrict__ idxs){
    if (m<=0)
        return;
...
int farthestpointsamplingLauncher(int b, int n, int m, const float * inp, float * temp, long * out, cudaStream_t stream){
    ...
    cudaError_t err;
    farthestpointsamplingKernel<<<32, 512, 0, stream>>>(b, n, m, inp, temp, out);
